...
class CentralExtractFan(FanEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self):
        ...
        if previous:
            ...
            if manual == 0 and not self._off_enabled: manual = 1
    ...
    async def async_set_percentage(self, percentage): await self.controller.async_set_manual_level(1 if percentage < 50 else 2 if percentage < 84 else 3)
    async def async_turn_on(self, **kwargs): await self.controller.async_set_manual_level(1)
    async def async_turn_off(self, **kwargs): await self.controller.async_set_manual_level(0 if self._off_enabled else 1)
